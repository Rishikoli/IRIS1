      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-[1.1rem] z-[2]">
          ...
          <div className="logo-container flex items-center md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 order-1 md:order-none">
            <img src={logo || "/logo.png"} alt={logoAlt} className="logo h-[100px]" />
          </div>
        </div>
        ...
        <div
          className={`card-nav-content absolute left-0 right-0 top-[60px] bottom-0 p-2 flex flex-col items-stretch gap-2 justify-start z-[1] ${isExpanded ? 'visible pointer-events-auto' : 'invisible pointer-events-none'
            } md:visible md:pointer-events-auto md:flex-row md:items-end md:gap-[12px]`}
          ...
          {(items || []).slice(0, 3).map((item, idx) => (
            ...
            >
              <div className="nav-card-label font-normal tracking-[-0.5px] text-[18px] md:text-[22px]">
                <DecodeText text={item.label} trigger={isExpanded || isDesktop} loop={true} />
              </div>
